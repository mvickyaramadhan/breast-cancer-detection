...
def train(**kwargs):
	...
	#model = getattr(models, opt.model)() # remark 10162020
	...
	for epoch in range(opt.max_epoch):
		...
		v_loss,v_accuracy,metric = val(model,val_loader,dataset_sizes['val'])
	...
	# 			   lr=lr))
		...
		if tpr1 > best_tpr:
			...
			#model.save(name = 'checkpoints/'+opt.model+'/'+str(epoch)+'.pth') # remark 10162020
			torch.save(model,'checkpoints/'+opt.model+'/'+str(epoch)+'.pth')
...
def val(model,dataloader,data_len):
	...
	return val_loss,val_accuracy,metric #confusion_matrix
